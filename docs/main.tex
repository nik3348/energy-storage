...
\emph{[Adaptation runs are in progress; this chapter carries the settled budget and robustness numbers, the partial adaptation results available so far, and placeholders for the rest. All quoted percentages are of the hindsight optimum on the real simulator, 20 paired 14-day episodes, unless stated otherwise.]}
...
Partial results are available for the fuel-step shift, where the reference arms have completed all 10 paired deployments. Pooled over seeds, the frozen dyna policy captures 91.5\% of the instantly-adapting oracle's net profit over the 30 calm pre-shift days but only 62.6\% over the 90 shifted days. The frozen unlimited-data policy falls farther, from 92.7\% to 53.9\%. The heuristic drops from 56.2\% to 36.6\%. Two observations already stand. First, both learned frozen policies capture over 90\% of oracle value in the calm regime, so what the adaptive arm must repair is the shift, not any baseline incompetence. Second, the frozen dyna policy holds up better under the fuel shift than the policy trained on unlimited calm data, reproducing the zero-shot ordering of Section~\ref{sec:results-robustness} in a streaming deployment. \pending{online-ft and adaptive-dyna arms for fuel-step (runs in progress); all arms for capacity-loss and cold-regime; capture curves, recovery-lag and cumulative-regret table, and the detection trace}
...
What the pending adaptation results must settle is attribution: how much of the adaptive arm's recovery comes from the market-model refit versus the policy fine-tune; whether nightly-always adaptation ever hurts through churn on calm days; and what the NLL trace implies about a surprise-triggered or surprise-scaled variant. \pending{discussion of the adaptation results against the pre-registered hypotheses of Section~\ref{sec:setup-adaptation}}
