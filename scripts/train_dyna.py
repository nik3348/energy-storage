#!/usr/bin/env python
"""Run one arm of the Dyna sample-efficiency experiment (docs/dyna-design.md).
...
def load_or_collect_history(path: Path, days: int, seed: int) -> MarketHistory:
    if path.exists():
        ...
        assert len(history) == days, f"{path} holds {len(history)} days, expected {days}"
        return history
...
def main() -> None:
    ...
    parser.add_argument("--history", type=Path, default=None, help="default data/history-d{D}-s{seed}.npz")
    parser.add_argument("--model", type=Path, default=None, help="gated ensemble from validate_market_model.py; default models/market-model-d{D}-s{seed}.pt")
    parser.add_argument("--eval-freq", type=int, default=None, help="default: timesteps/25")
    parser.add_argument("--eval-episodes", type=int, default=20)
    parser.add_argument("--models-dir", type=Path, default=None, help="default models/dyna/{arm}-d{D}-s{seed}")
    parser.add_argument("--wandb", action=argparse.BooleanOptionalAction, default=True)
    args = parser.parse_args()

    d = args.budget_days
    history_path = args.history or Path("data") / f"history-d{d}-s{args.seed}.npz"
    models_dir = args.models_dir or Path("models") / "dyna" / f"{args.arm}-d{d}-s{args.seed}"
    ...
    train_env = make_vec_env(make_env_fn(train_config), n_envs=args.n_envs, seed=args.seed)
    eval_env = make_vec_env(make_env_fn(train_config), n_envs=1, seed=args.seed + 10_000)
    ...
    hindsight = evaluate_hindsight(real_config, episodes=args.eval_episodes, seed0=1_000_000)
    policies = [("idle", idle_policy), ("heuristic", heuristic_policy), ("final", model_policy(model))]
    ...
    for name, policy in policies:
        ...
        pct = 100.0 * stats["net"] / hindsight["net"] if hindsight["net"] else float("nan")
        print(f"{name:<12} {stats['net']:>9.2f}$ {pct:>11.1f}%")
        if run:
            import wandb

            wandb.log({f"real_eval/{name}_{k}": v for k, v in stats.items()}
