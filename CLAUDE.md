...
2. **`energy_storage/market/`** — the price simulator. `Market.simulate_day()` in `day_ahead.py` orchestrates one day: Calendar/Weather drivers → Demand (temperature-sensitive, U-shaped) and FuelMarket (OU processes) → each Generator submits `(price, quantity)` offers per hour → `clearing.clear_hour()` runs a uniform-price merit-order auction → `DayResult` (24 prices + dispatch by tech). Price formation is causal: shocks hit drivers, never prices directly. Negative prices emerge from subsidy-style negative renewable bids, coal min-run blocks, and must-run geothermal; scarcity hours clear at `price_cap`. Hydro bids its opportunity cost as a function of reservoir fill (self-stabilizing).
...
Market tests are statistical assertions over a seeded simulated year (fixture in `tests/test_market.py`): median price ~$58/MWh, evening peak > overnight, scarcity < 1.5% of hours, negative prices rare but present. If you touch fleet capacities, weather parameters, or bid levels, these tests are the calibration gate — a failure usually means the market drifted, not that the test is wrong. The generator fleet is sized so scarcity comes from outage+calm+peak coincidences, not routine winter evenings.

Benchmark context for evaluating agents (net $/14-day episode, 20 paired seeds at seed0=1,000,000): hindsight optimum ~+$62 (=100%), rolling-horizon oracle ~+$62 (99.8% — the 24h window is near-sufficient information in the calm market), SAC best_model ~+$52 (83%), heuristic ~+$16 (25%), idle ~−$13 (calendar aging is unavoidable). Report policies as % of the hindsight optimum, not raw dollars: per-episode variance across market-weeks is huge (~10x between seed blocks — scarcity weeks dominate), so always compare paired same-seed over ≥20 episodes.
