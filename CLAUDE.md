...
`energy_storage/baselines.py` has the idle/heuristic policies and rollout/eval helpers shared by both scripts — extend there, not in the scripts.
...
Benchmark context for evaluating agents: the overnight-charge/evening-discharge heuristic nets ~+$16 per 14-day episode on held-out seeds, idle ~−$13 (calendar aging is unavoidable). Per-episode variance across market-weeks is high; compare policies on ≥20 seeded episodes.
