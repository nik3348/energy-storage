...
uv run --extra train python scripts/pipeline/train_reference_policy.py  # trains, evals vs baselines
uv run --extra train python scripts/results/measure_budget_sweep.py     # benchmark ladder; skips any arm with no checkpoint
```

`scripts/` splits into `pipeline/` (collects data, trains checkpoints — produces no paper number directly) and `results/` (one script per table/figure/in-text result in `docs/main.tex`). README.md maps each result script to what it produces. The training scripts log to the `energy-storage` wandb project by default; pass `--no-wandb` to skip.
...
Benchmark context for evaluating agents (net $/14-day episode, 20 paired seeds at seed0=1,000,000, European market, `price_cap=1000`): hindsight optimum +$28 (=100%), rolling-horizon oracle +$28 (99.9% — the 24h window is near-sufficient information), heuristic −$3 (over-cycles; above idle but net-negative), idle −$13 (calendar aging is unavoidable). The dollar numbers scale with `price_cap` because most arbitrage value is scarcity capture (at cap 3000 hindsight was ~+$90; at cap 200 the heuristic drops below idle) — 1000 was chosen so the ladder stays ordered and eval variance stays manageable; re-measure if the cap changes. SAC models trained before the European market redesign or the observation slim-down (weather and fuel features removed; both July 2026) are stale — old checkpoints don't even match the current obs shape. Retrain before quoting numbers. Report policies as % of the hindsight optimum, not raw dollars: per-episode variance across market-weeks is huge (scarcity weeks dominate), so always compare paired same-seed over ≥20 episodes.
